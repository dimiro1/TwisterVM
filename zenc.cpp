...
void createFile(char *progname, int size)
{
...
void writeData(char *progname)
{
    ...
    CompiledBytecode *cp = new CompiledBytecode(progname, 8);
    cp->instructions[0] = ByteCode(PUSH, 8);
    cp->instructions[1] = ByteCode(PUSH, 6);
    cp->instructions[2] = ByteCode(ADD);
    cp->instructions[3] = ByteCode(PUSH, 2);
    cp->instructions[4] = ByteCode(MULT);
    cp->instructions[5] = ByteCode(PUSH, 5);
    cp->instructions[6] = ByteCode(MULT);
    cp->instructions[7] = ByteCode(PUTS);
...
void read(char *progname)
{
    ...
    for(int i = 0; i < cp->size; i++) {
        ...
        if( cp->instructions[i].operand != 0 )
            printf ("| %-2d | %-15s |  %-3d |", i+1, opcodeName[cp->instructions[i].opcode], cp->instructions[i].operand);
        else
